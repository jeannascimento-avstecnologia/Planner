-- Fix: update_card_fields rejeitava board-only members (convite sem memberships.org)
-- can_write_board OK (anexos/edit), mas v_role vinha só de memberships → forbidden.

create or replace function app.resolve_card_field_role(p_org uuid, p_board uuid)
...
begin
  ...
  -- Dept-only writers (sem org/board membership): defaults de manager para field ACL.
  if v_role is null and app.can_write_board(p_board) then
    v_role := 'manager'::public.membership_role;
  end if;
...
grant execute on function app.resolve_card_field_role(uuid, uuid) to authenticated;
...
begin
  ...
  v_role := app.resolve_card_field_role(v_card.org_id, v_card.board_id);
