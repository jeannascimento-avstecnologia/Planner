-- F.2 update_card_fields RPC + guard

create or replace function app.field_access(p_org uuid, p_role public.membership_role, p_field text)
returns text
language sql
stable
security definer
...
as $$
  select coalesce(
    (select fpr.access from public.field_permission_rules fpr
     where fpr.org_id = p_org and fpr.role = p_role and fpr.resource = 'card' and fpr.field_name = p_field),
    'write'
  );
$$;
