...
export type OrgSwitcherTestIds = {
  root: string;
  trigger: string;
  menu: string;
  optionPrefix: string;
...
type Props = {
  ...
  orgs: UserOrgRow[];
  testIds: OrgSwitcherTestIds;
  className?: string;
};

const blockClass =
  "flex w-full items-center gap-3 rounded-xl border border-aurora-border bg-aurora-surface px-3 py-2.5 text-left shadow-sm";

function OrgLabel({ name }: { name: string }) {
  return (
    <span className="min-w-0 flex-1">
      <span className="block truncate text-sm font-semibold text-aurora-fg">{name}</span>
...
export function OrgSwitcher({
  ...
  return (
    <div ref={ref} className={`relative ${className ?? ""}`} data-testid={testIds.root}>
      {canSwitch ? (
        <button
          type="button"
          aria-expanded={open}
          ...
          disabled={pending}
          onClick={() => setOpen((v) => !v)}
          className={`${blockClass} ${focusRingAurora} hover:border-aurora-brand/40 hover:bg-aurora-surface-2 disabled:opacity-70`}
          data-testid={testIds.trigger}
        >
          <OrgLogo name={activeOrgName} logoUrl={activeOrgLogoUrl} size="sm" />
          <OrgLabel name={activeOrgName} />
          <ChevronDown className={`h-4 w-4 shrink-0 text-aurora-muted transition ${open ? "rotate-180" : ""}`} />
        </button>
      ) : (
        <div className={blockClass} data-testid={testIds.trigger}>
          <OrgLogo name={activeOrgName} logoUrl={activeOrgLogoUrl} size="sm" />
          <OrgLabel name={activeOrgName} />
        </div>
      )}
            ...
            return (
              <li key={org.orgId} role="option" aria-selected={selected}>
                <button
                  ...
                  data-testid={`${testIds.optionPrefix}-${org.orgId}`}
                >
