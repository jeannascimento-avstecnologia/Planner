...
import { WorkloadOrgSwitcher } from "@/components/workload/workload-org-switcher";
  ...
  return (
    <>
      <div data-tour="workload-header">
        ...
        />
        <div className="mt-4" data-tour="workload-org-switcher">
          {orgSwitcher}
        </div>
      </div>
      {toolbar}
...
export default async function WorkloadPage({ searchParams }: Props) {
  ...
  const [orgId, userOrgs] = await Promise.all([getActiveOrgId(), listUserOrgs()]);
  ...
  const workloadOrgs = userOrgs.filter((org: UserOrgRow) => canViewWorkload(org.role));
  const activeOrg = workloadOrgs.find((org) => org.orgId === orgId) ?? workloadOrgs[0];
  ...
  const orgSwitcher = activeOrg ? (
    <WorkloadOrgSwitcher
      activeOrgId={activeOrg.orgId}
      activeOrgName={activeOrg.name}
      activeOrgLogoUrl={activeOrg.logoUrl}
      orgs={workloadOrgs}
    />
  ) : null;
